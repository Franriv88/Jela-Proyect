document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Función para validar formulario y habilitar botón ---
    ...
    // Cargar Disponibilidad (Días completos)
    try {
        if (typeof db !== 'undefined') {
            const snapshot = await db.collection('blockedItems').get();
            snapshot.docs.forEach(doc => {
                const item = doc.data();
                // Si tiene fecha, la añadimos a la lista de bloqueos
                if (item.date) { 
    ...
    // --- INICIALIZAR CALENDARIO (SOLO FECHA) ---
    const minDate48Hours = new Date(Date.now() + 48 * 60 * 60 * 1000); 
    
    flatpickr("#fecha", { 
        enableTime: false, // <--- DESHABILITADO EL SELECTOR DE HORA
        dateFormat: "Y-m-d", // <--- FORMATO SOLO FECHA
        locale: "es", 
    ...
    if(bookingForm){
        ...
        bookingForm.addEventListener('submit', async (e) => {
            ...
            // No guardamos 'phone' si no existe el campo
            ...
            const reserva = {
                ...
                fecha: document.getElementById('fecha').value, // Solo fecha
            ...
            try {
                ...
                btnConfirm.disabled = true; // Se deshabilita hasta nueva validación
